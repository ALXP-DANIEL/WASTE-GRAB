...
import type { GooglePlaceSelection, PlaceAutocompleteResponse } from '@wastegrab/shared';
...
export class PlaceSearchService {
  ...
  autocomplete(input: string, country: string): Observable<PlaceAutocompleteResponse> {
    const params = new HttpParams().set('input', input).set('country', country);
    return this.http.get<PlaceAutocompleteResponse>(`${this.apiUrl}/autocomplete`, {
      ...this.options,
      params,
    });
  }
