...
import { UserRole, type Address, type CreateAddressInput } from '@wastegrab/shared';
...
const ONBOARDING_STEPS: { key: CustomerOnboardingStep; label: string; icon: string }[] = [
...
@Component({
  selector: 'app-layout',
  imports: [AppNavbarComponent, RouterOutlet, NgClass, NgIcon, ZardButtonComponent, ReactiveFormsModule],
  viewProviders: [
    provideIcons({
      lucideBell,
      lucideCheckCircle2,
              ...
              @case ('welcome') {
                <div class="grid gap-3 sm:grid-cols-3">
                  <div class="rounded-lg border border-border bg-card p-4">
                    ...
                    <p class="mt-1 text-sm leading-5 text-muted-foreground">Use it faster when creating a request.</p>
                  </div>
                  <div class="rounded-lg border border-border bg-card p-4">
                    ...
                    <p class="mt-1 text-sm leading-5 text-muted-foreground">Know when the collector accepts or completes the pickup.</p>
                  </div>
                  <div class="rounded-lg border border-border bg-card p-4">
                    ...
                    <p class="mt-1 text-sm leading-5 text-muted-foreground">Install the app when supported and choose a theme.</p>
                  </div>
                </div>
              }
...
export class AppLayout {
  ...
  protected readonly shouldShowCustomerOnboarding = computed(() => {
    ...
    return user?.role === UserRole.CUSTOMER && !user.hasCompletedCustomerOnboarding;
  });
  ...
  constructor() {
    effect(() => {
      ...
      if (!this.shouldShowCustomerOnboarding() || !user || this.loadedForUserId === user.id) {
        return;
      }

      this.loadedForUserId = user.id;
      this.activeStep.set('welcome');
  ...
  protected async finishOnboarding(destination?: 'pickup'): Promise<void> {
    ...
    try {
      await firstValueFrom(this.authService.completeCustomerOnboarding());
