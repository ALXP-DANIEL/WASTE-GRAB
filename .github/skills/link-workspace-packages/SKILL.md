...
description: 'Link workspace packages in monorepos (npm, yarn, pnpm, bun). USE WHEN: (1) you just created or generated new packages and need to wire up their dependencies, (2) user imports from a sibling package and needs to add it as a dependency, (3) you get resolution errors for workspace packages (@wastegrab/*) like "cannot find module", "failed to resolve import", "TS2307", or "cannot resolve". DO NOT patch around with tsconfig paths or manual package.json edits - use the package manager''s workspace commands to fix actual linking.'
---
...
pnpm add @wastegrab/ui --workspace

# Or with --filter from anywhere
pnpm add @wastegrab/ui --filter @wastegrab/app --workspace
```

Result in `package.json`:

```json
{ "dependencies": { "@wastegrab/ui": "workspace:*" } }
```
...
```bash
yarn workspace @wastegrab/app add @wastegrab/ui
```
...
```bash
npm install @wastegrab/ui --workspace @wastegrab/app
```

Result in `package.json`:

```json
{ "dependencies": { "@wastegrab/ui": "*" } }
```
...
```bash
cd packages/app && bun add @wastegrab/ui
```
...
```bash
npm install @wastegrab/data-access @wastegrab/ui --workspace @wastegrab/dashboard
```
...
- Symlinks appear in `<consumer>/node_modules/@wastegrab/<package>`
